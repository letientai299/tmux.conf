...
tmux \
  ...
  set -g status-right \
    "#[fg=$c_blue]#(echo \${USER:-\$LOGNAME})#[fg=$c_muted]@#[fg=$c_fg]#H#[fg=$c_muted]│#[fg=$c_green]#(~/.config/tmux/lib/pub-ip.sh)#[fg=$c_muted]│#[fg=$c_yellow]#(~/.config/tmux/lib/uptime.sh)#[fg=$c_muted]│#[fg=$c_fg]%H:%M " \; \
