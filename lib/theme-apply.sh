...
tmux \
  ...
  set -g status-right \
    "#[fg=$c_blue]#(echo \${USER:-\$LOGNAME})#[fg=$c_muted]@#[fg=$c_fg]#H#[fg=$c_muted]│#[fg=$c_green]#(\$TMUX_CONF_DIR/lib/pub-ip.sh)#[fg=$c_muted]│#[fg=$c_yellow]#(\$TMUX_CONF_DIR/lib/uptime.sh)#[fg=$c_muted]│#[fg=$c_fg]%H:%M " \; \
